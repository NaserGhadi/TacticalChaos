...
public class KassadinAbility extends Ability {
    @Override
    public synchronized Object doTheAbility(String playerName,int heroFrequentNumber) {
        ...
        if (checkIfConsolePlayer(playerName)){
            ...
            System.out.println("Which player you want to steal his mana from his hero and damage him");
            for (int i=0;i<Game.getPlayers().size();i++){
                for (int j=0;j<Game.getPlayers().get(i).getHeroes().size();j++){
    ...
    public KassadinAbility(){
        this.abilityName = "KassadinAbility";
        this.heroID = 17;
    }
}
