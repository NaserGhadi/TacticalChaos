...
public class MordekaiserAbility extends Ability {
    public MordekaiserAbility(){
        this.abilityName = "MordekaiserAbility";
        this.heroID = 28;
    }
    @Override
                ...
                for (int j = 0; j < Game.getPlayers().get(i).getHeroes().size(); j++) {
                    if (this.heroID == Game.getPlayers().get(i).getHeroes().get(j).getAttr().getID()){
                        if (frequent == heroFrequentNumber){
                            Game.getPlayers().get(i).getHeroes().get(j).getAttr().incrementDamage(150);
